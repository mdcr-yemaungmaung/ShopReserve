/* ============================================================
   EzBookNow Screen AD-17 / S-10 — Security Settings Screen
   Fully aligned with prototype/DESIGN.md & AD-18 standard (Components.icon())
   - Colors: Primary (#0F4C5C), Secondary (#D8902F), Tertiary (#623013), Neutral (#1F2937)
   - Glassmorphism: Backdrop blur (16px), 1px solid white 10% opacity top/left border
   - High Contrast CTA: Brand Primary & Secondary (#0F4C5C, #D8902F)
...
const ScreenS10 = (() => {
  ...
  function render() {
    ...
    const headerHtml = `
      <div style="background:rgba(15,76,92,0.95); backdrop-filter:blur(16px); color:#ffffff; border-radius:12px; padding:24px 28px; margin-bottom:24px; border:1px solid #c7c5d0; border-top:1px solid rgba(255,255,255,0.8); border-left:1px solid rgba(255,255,255,0.8); font-family:'Inter',sans-serif;">
        <div style="display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:16px;">
          <div>
            ...
            <h1 style="font-size:24px; font-weight:700; font-family:'Outfit',sans-serif; margin:0 0 8px 0; color:#ffffff; display:flex; align-items:center; gap:12px;">
              <span style="color:#D8902F; display:inline-flex;">${getIcon('shield', 26)}</span>
          ...
          <div style="background:rgba(255,255,255,0.1); backdrop-filter:blur(12px); border:1px solid rgba(255,255,255,0.2); padding:14px 20px; border-radius:12px; display:flex; align-items:center; gap:16px; min-width:260px;">
            <div style="width:48px; height:48px; display:flex; align-items:center; justify-content:center; background:${securityScore >= 80 ? '#D8902F' : '#fff7ed'}; color:${securityScore >= 80 ? '#ffffff' : '#c2410c'}; border-radius:9999px; font-family:'Outfit',sans-serif; font-size:16px; font-weight:700;">
              ${securityScore}%
            </div>
            <div>
    ...
    const tabsBarHtml = `
      <div style="display:flex; gap:8px; background:#f3f4f5; padding:4px; border-radius:9999px; margin-bottom:24px; border:1px solid #c7c5d0; overflow-x:auto; font-family:'Inter',sans-serif;">
        <button onclick="ScreenS10.switchTab('password')" 
                style="flex:1; padding:10px 16px; border:none; border-radius:9999px; font-size:13px; font-weight:600; font-family:'Inter',sans-serif; cursor:pointer; transition:all 0.15s; display:flex; align-items:center; justify-content:center; gap:8px; ${activeTab === 'password' ? 'background:#0F4C5C; color:#ffffff;' : 'background:transparent; color:#46464f;'}">
          ${getIcon('key', 16)} ${I18n.t('change_password_title')}
        </button>

        <button onclick="ScreenS10.switchTab('2fa')" 
                style="flex:1; padding:10px 16px; border:none; border-radius:9999px; font-size:13px; font-weight:600; font-family:'Inter',sans-serif; cursor:pointer; transition:all 0.15s; display:flex; align-items:center; justify-content:center; gap:8px; ${activeTab === '2fa' ? 'background:#0F4C5C; color:#ffffff;' : 'background:transparent; color:#46464f;'}">
          ${getIcon('shield', 16)} ${I18n.t('two_factor_title')}
          ${twofaEnabled ? '<span style="width:8px; height:8px; border-radius:9999px; background:#D8902F; display:inline-block;"></span>' : '<span style="width:8px; height:8px; border-radius:9999px; background:#ba1a1a; display:inline-block;"></span>'}
        </button>

        <button onclick="ScreenS10.switchTab('sessions')" 
                style="flex:1; padding:10px 16px; border:none; border-radius:9999px; font-size:13px; font-weight:600; font-family:'Inter',sans-serif; cursor:pointer; transition:all 0.15s; display:flex; align-items:center; justify-content:center; gap:8px; ${activeTab === 'sessions' ? 'background:#0F4C5C; color:#ffffff;' : 'background:transparent; color:#46464f;'}">
          ${getIcon('clipboard', 16)} Sessions & Audit Log
        </button>

        <button onclick="ScreenS10.switchTab('policy')" 
                style="flex:1; padding:10px 16px; border:none; border-radius:9999px; font-size:13px; font-weight:600; font-family:'Inter',sans-serif; cursor:pointer; transition:all 0.15s; display:flex; align-items:center; justify-content:center; gap:8px; ${activeTab === 'policy' ? 'background:#0F4C5C; color:#ffffff;' : 'background:transparent; color:#46464f;'}">
    ...
    const tabPasswordHtml = `
      <div style="border-radius:12px; padding:28px; border:1px solid rgba(15,76,92,0.14); background:linear-gradient(145deg, rgba(251,252,254,0.95) 0%, rgba(240,244,247,0.9) 100%); backdrop-filter:blur(16px); box-shadow:0 2px 8px rgba(15,76,92,0.04); font-family:'Inter',sans-serif;">
        <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:20px; border-bottom:1px solid rgba(15,76,92,0.1); padding-bottom:14px;">
          <div>
            <h3 style="font-size:18px; font-weight:700; font-family:'Outfit',sans-serif; color:#191c1d; margin:0; display:flex; align-items:center; gap:8px;">
              <span style="color:#0F4C5C; display:inline-flex;">${getIcon('key', 20)}</span>
        ...
        <form onsubmit="ScreenS10.handlePasswordUpdate(event)">
            ...
            <div style="margin:0;">
              ...
              <div style="position:relative;">
                <input type="${showCurrentPwd ? 'text' : 'password'}" class="form-input" id="pwd-current" required placeholder="••••••••" 
                       style="padding-right:45px; font-size:14px; height:42px; border-radius:8px; border:1px solid #c7c5d0; background:#f4f8fa; outline:none; width:100%; box-sizing:border-box;"
                       onfocus="this.style.borderColor='#0F4C5C'" onblur="this.style.borderColor='#c7c5d0'"
          ...
          <div style="background:#f8f9fa; border:1px solid #c7c5d0; border-radius:12px; padding:20px; margin-bottom:24px;">
            ...
            <div style="margin-top:16px; font-size:12px; color:#46464f; line-height:1.5; border-top:1px dashed #c7c5d0; padding-top:12px; display:flex; align-items:center; gap:8px;">
              <span style="color:#0F4C5C; display:inline-flex;">${getIcon('lock', 16)}</span>
          ...
          <div style="display:flex; justify-content:flex-end;">
            <button type="submit" id="pwd-save-btn" disabled 
                    style="padding:10px 24px; font-size:14px; font-weight:700; font-family:'Inter',sans-serif; border-radius:8px; border:none; background:#0F4C5C; color:#ffffff; cursor:pointer; display:inline-flex; align-items:center; gap:6px; box-shadow:0 3px 10px rgba(15,76,92,0.3);">
        ...
        <div style="background:#f8f9fa; border:1px solid #c7c5d0; border-radius:12px; padding:24px; margin-top:20px;">
          <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:14px;">
            <h4 style="font-size:16px; font-weight:700; font-family:'Outfit',sans-serif; color:#191c1d; margin:0; display:flex; align-items:center; gap:8px;">
              <span style="color:#0F4C5C; display:inline-flex;">${getIcon('shield', 20)}</span>
              Multi-Factor Authentication Setup (TOTP)
            </h4>
            <span style="font-size:12px; background:#0F4C5C; color:#D8902F; padding:4px 12px; border-radius:9999px; font-weight:700;">
              Step 1 of 2
            </span>
          </div>

          <p style="font-size:13.5px; color:#46464f; line-height:1.6; margin:0 0 18px 0;">
            <div style="display:flex; flex-direction:row; gap:24px; align-items:center; flex-wrap:wrap; background:#f4f8fa; padding:20px; border-radius:12px; border:1px solid rgba(15,76,92,0.14); margin-bottom:18px;">
            
            <!-- Styled SVG QR Code -->
            <div style="background:#edf3f7; padding:14px; border-radius:12px; border:2px solid #0F4C5C; display:inline-flex; flex-direction:column; align-items:center; justify-content:center;">
              <svg width="120" height="120" viewBox="0 0 100 100" style="display:block;">
                <rect width="100" height="100" fill="#edf3f7"/>
                <!-- Outer Corners -->
                <rect x="5" y="5" width="25" height="25" fill="#0F4C5C"/>
                <rect x="8" y="8" width="19" height="19" fill="#edf3f7"/>
                <rect x="12" y="12" width="11" height="11" fill="#0F4C5C"/>

                <rect x="70" y="5" width="25" height="25" fill="#0F4C5C"/>
                <rect x="73" y="8" width="19" height="19" fill="#edf3f7"/>
                <rect x="77" y="12" width="11" height="11" fill="#0F4C5C"/>

                <rect x="5" y="70" width="25" height="25" fill="#0F4C5C"/>
                <rect x="8" y="73" width="19" height="19" fill="#edf3f7"/>
                <rect x="12" y="77" width="11" height="11" fill="#0F4C5C"/>

                <!-- QR Matrix Dots -->
                <rect x="35" y="10" width="8" height="8" fill="#0F4C5C"/>
                <rect x="50" y="10" width="8" height="8" fill="#0F4C5C"/>
                <rect x="35" y="25" width="15" height="8" fill="#0F4C5C"/>
                <rect x="10" y="35" width="12" height="12" fill="#0F4C5C"/>
                <rect x="28" y="40" width="10" height="10" fill="#0F4C5C"/>
                <rect x="45" y="35" width="15" height="15" fill="#0F4C5C"/>
                <rect x="68" y="38" width="12" height="12" fill="#0F4C5C"/>
                <rect x="85" y="35" width="10" height="15" fill="#0F4C5C"/>
                <rect x="35" y="55" width="20" height="10" fill="#0F4C5C"/>
                <rect x="60" y="55" width="10" height="10" fill="#0F4C5C"/>
                <rect x="75" y="55" width="15" height="15" fill="#0F4C5C"/>
                <rect x="35" y="70" width="10" height="20" fill="#0F4C5C"/>
                <rect x="50" y="75" width="15" height="10" fill="#0F4C5C"/>
                <rect x="70" y="75" width="20" height="15" fill="#0F4C5C"/>
              </svg>
            ...
            <div style="flex:1; min-width:260px;">
              ...
              <div style="display:flex; align-items:center; gap:10px; margin-bottom:16px;">
                <code style="padding:10px 16px; background:#edf3f7; border-radius:8px; font-size:15px; color:#0F4C5C; font-family:monospace; font-weight:700; border:1px solid #c7c5d0; letter-spacing:0.12em;">
                  EZBK SHP1 SECR ETM2
                </code>
                <button type="button" onclick="ScreenS10.copySecretKey('EZBKSHP1SECRETM2')" 
                        style="font-size:13px; border:1px solid #c7c5d0; background:#f4f8fa; color:#0F4C5C; font-weight:600; font-family:'Inter',sans-serif; border-radius:8px; padding:8px 14px; cursor:pointer; display:inline-flex; align-items:center; gap:4px;">
              ...
              <div style="margin:0;">
                ...
                <div style="display:flex; gap:12px; align-items:center;">
                  <input type="text" class="form-input" id="totp-code" placeholder="123456" maxlength="6" pattern="\\d{6}" 
                         style="font-size:18px; text-align:center; letter-spacing:0.25em; height:44px; width:180px; font-weight:700; border-radius:8px; border:1px solid #c7c5d0; background:#f4f8fa; outline:none;"
                         onfocus="this.style.borderColor='#0F4C5C'" onblur="this.style.borderColor='#c7c5d0'"
                         oninput="ScreenS10.validateTOTPForm(); ScreenS10.clearTOTPAlert()">
                  <button type="button" id="totp-verify-btn" disabled 
                          style="height:44px; font-size:14px; padding:0 24px; font-weight:700; font-family:'Inter',sans-serif; border-radius:8px; border:none; background:#0F4C5C; color:#ffffff; cursor:pointer; display:inline-flex; align-items:center; gap:6px; box-shadow:0 3px 10px rgba(15,76,92,0.3);"
                          onclick="ScreenS10.verify2FAActivation()">
                    ${getIcon('check', 18)} ${I18n.t('confirm')}
                  </button>
                </div>
                <p style="font-size:12px; color:#777680; margin:6px 0 0 0;">
                  Test Code: <code style="font-weight:700; color:#0F4C5C;">123456</code>
                </p>
              </div>
            </div>
    ...
    const tab2FAHtml = `
      <div style="border-radius:12px; padding:28px; border:1px solid rgba(15,76,92,0.14); background:linear-gradient(145deg, rgba(251,252,254,0.95) 0%, rgba(240,244,247,0.9) 100%); backdrop-filter:blur(16px); box-shadow:0 2px 8px rgba(15,76,92,0.04); font-family:'Inter',sans-serif;">
        <div style="display:flex; justify-content:space-between; align-items:flex-start; margin-bottom:18px; flex-wrap:wrap; gap:14px;">
          ...
          <span style="font-size:12px; font-weight:700; padding:6px 16px; border-radius:9999px; display:inline-flex; align-items:center; gap:6px; ${twofaEnabled ? 'background:#d0e6ec; color:#0F4C5C; border:1px solid #0F4C5C;' : 'background:#ffdad6; color:#93000a; border:1px solid #fca5a5;'}">
        ...
        <div style="display:grid; grid-template-columns: 1fr 1fr; gap:14px; margin-bottom:20px;" class="form-row-responsive">
          <div style="background:#f0f4f7; border:1px solid rgba(15,76,92,0.12); border-radius:12px; padding:16px;">
          ...
          <div style="background:${isPhase2 ? '#fff7ed' : '#f0f4f7'}; border:1px solid ${isPhase2 ? '#fed7aa' : 'rgba(15,76,92,0.12)'}; border-radius:12px; padding:16px;">
        ...
        ${(!showSetupWizard && !showDeactivateWizard) ? `
          <div style="display:flex; justify-content:flex-end; margin-top:14px;">
            <button style="padding:10px 24px; font-size:14px; font-weight:700; font-family:'Inter',sans-serif; border-radius:8px; ${twofaEnabled ? 'background:#f4f8fa; color:#46464f; border:1px solid #c7c5d0;' : 'background:#0F4C5C; color:#ffffff; border:none; box-shadow:0 3px 10px rgba(15,76,92,0.3);'} cursor:pointer; display:inline-flex; align-items:center; gap:6px;"
        ...
        ${(twofaEnabled) ? `
          <div style="background:#d0e6ec; border:1px solid #0F4C5C; border-radius:12px; padding:20px; margin-top:20px;">
            <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:10px;">
              <div style="font-size:14px; font-weight:700; font-family:'Outfit',sans-serif; color:#0F4C5C; display:flex; align-items:center; gap:8px;">
                ${getIcon('shield', 18)} ${I18n.t('backup_codes_title')}
              </div>
              <span style="font-size:11.5px; background:#f4f8fa; color:#0F4C5C; font-weight:700; padding:4px 10px; border-radius:9999px; border:1px solid #0F4C5C;">
                8 Single-Use Codes Ready
              </span>
            </div>
            <p style="font-size:13px; color:#0F4C5C; margin:0 0 14px 0; line-height:1.5;">
              ${I18n.t('backup_codes_desc')}
            </p>

            <div style="display:grid; grid-template-columns: repeat(4, 1fr); gap:10px; margin-bottom:16px; background:#f4f8fa; padding:14px; border-radius:8px; border:1px solid #0F4C5C;">
              ${RECOVERY_CODES.map(c => `
                <div style="font-family:monospace; font-size:14px; font-weight:700; color:#191c1d; text-align:center; background:#edf3f7; padding:8px; border-radius:6px; border:1px solid #c7c5d0; letter-spacing:0.05em;">
                  ${c}
                </div>
              `).join('')}
            </div>
            ...
            <div style="display:flex; gap:12px; flex-wrap:wrap;">
              <button style="font-size:13px; font-weight:600; font-family:'Inter',sans-serif; background:#f4f8fa; border:1px solid #0F4C5C; color:#0F4C5C; border-radius:8px; padding:8px 16px; cursor:pointer; display:inline-flex; align-items:center; gap:6px;" onclick="ScreenS10.copyBackupCodes()">
                ${getIcon('clipboard', 14)} ${I18n.t('copy_backup_codes')}
              </button>
              <button style="font-size:13px; font-weight:600; font-family:'Inter',sans-serif; background:#f4f8fa; border:1px solid #0F4C5C; color:#0F4C5C; border-radius:8px; padding:8px 16px; cursor:pointer; display:inline-flex; align-items:center; gap:6px;" onclick="ScreenS10.downloadBackupCodes()">
    ...
    const tabSessionsHtml = `
      <div style="border-radius:12px; padding:28px; border:1px solid rgba(15,76,92,0.14); background:linear-gradient(145deg, rgba(251,252,254,0.95) 0%, rgba(240,244,247,0.9) 100%); backdrop-filter:blur(16px); box-shadow:0 2px 8px rgba(15,76,92,0.04); font-family:'Inter',sans-serif; margin-bottom:24px;">
        <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:18px; border-bottom:1px solid rgba(15,76,92,0.1); padding-bottom:14px;">
          <div>
            <h3 style="font-size:18px; font-weight:700; font-family:'Outfit',sans-serif; color:#191c1d; margin:0; display:flex; align-items:center; gap:8px;">
              <span style="color:#0F4C5C; display:inline-flex;">${getIcon('clipboard', 20)}</span>
        ...
        <div style="display:flex; flex-direction:column; gap:14px; margin-bottom:28px;">
          ...
          <div style="display:flex; justify-content:space-between; align-items:center; padding:14px 18px; background:#f4f8fa; border:1px solid rgba(15,76,92,0.14); border-radius:8px;">
            <div style="display:flex; align-items:center; gap:14px;">
              <span style="color:#0F4C5C; display:inline-flex;">${getIcon('globe', 24)}</span>
              <div>
                <div style="font-size:14px; font-weight:700; font-family:'Outfit',sans-serif; color:#191c1d; display:flex; align-items:center; gap:10px;">
                  ...
                  <span style="background:#d0e6ec; color:#0F4C5C; border:1px solid #0F4C5C; font-size:11px; font-weight:700; font-family:'Inter',sans-serif; padding:2px 10px; border-radius:9999px;">
                    Current Device
                  </span>
                </div>
            ...
            <span style="font-size:12px; color:#0F4C5C; font-weight:700; font-family:'Inter',sans-serif;">Online</span>
          </div>

          <div style="display:flex; justify-content:space-between; align-items:center; padding:14px 18px; background:#f4f8fa; border:1px solid rgba(15,76,92,0.14); border-radius:8px;">
            <div style="display:flex; align-items:center; gap:14px;">
              <span style="color:#0F4C5C; display:inline-flex;">${getIcon('phone', 24)}</span>
              <div>
        ...
        <div style="overflow-x:auto; border:1px solid rgba(15,76,92,0.14); border-radius:8px;">
          <table style="width:100%; border-collapse:collapse; text-align:left; font-size:13px; background:#f4f8fa;">
            <thead>
              <tr style="background:#edf3f7; border-bottom:1px solid rgba(15,76,92,0.14); color:#46464f; font-weight:600; font-size:12px; text-transform:uppercase; letter-spacing:0.05em;">
            ...
            <tbody>
              ${AUDIT_LOGS.map(log => `
                <tr style="border-bottom:1px solid rgba(15,76,92,0.08);">
                  <td style="padding:12px 14px; font-family:monospace; font-weight:700; color:#0F4C5C;">${log.id}</td>
                  <td style="padding:12px 14px; font-weight:700; font-family:'Outfit',sans-serif; color:#191c1d;">${log.event}</td>
                  <td style="padding:12px 14px;">
                    <span style="background:#edf3f7; color:#46464f; border:1px solid #c7c5d0; font-size:11px; font-weight:600; padding:2px 8px; border-radius:9999px;">
                      ${log.category}
                    </span>
                  </td>
                  ...
                  <td style="padding:12px 14px; text-align:right;">
                    <span style="font-size:11px; font-weight:700; padding:3px 10px; border-radius:9999px; ${log.status === 'Success' ? 'background:#d0e6ec; color:#0F4C5C; border:1px solid #0F4C5C;' : 'background:#ffdad6; color:#93000a; border:1px solid #fca5a5;'}">
                      ${log.status}
                    </span>
                  </td>
                </tr>
    ...
    const tabPolicyHtml = `
      <div style="border-radius:12px; padding:28px; border:1px solid rgba(15,76,92,0.14); background:linear-gradient(145deg, rgba(251,252,254,0.95) 0%, rgba(240,244,247,0.9) 100%); backdrop-filter:blur(16px); box-shadow:0 2px 8px rgba(15,76,92,0.04); font-family:'Inter',sans-serif;">
        <div style="font-weight:700; font-size:18px; font-family:'Outfit',sans-serif; color:#191c1d; margin-bottom:8px; display:flex; align-items:center; gap:8px;">
          <span style="color:#0F4C5C; display:inline-flex;">${getIcon('settings', 20)}</span>
        ...
        <div style="background:#f0f4f7; border:1px solid rgba(15,76,92,0.12); border-radius:8px; padding:20px; margin-bottom:18px;">
          <div style="margin-bottom:0; max-width:480px;">
            ...
            <select class="form-input" id="sim-security-phase" onchange="ScreenS10.changeSecurityPhase(this.value)" 
                    style="font-size:14px; font-family:'Inter',sans-serif; height:42px; padding:8px 14px; cursor:pointer; border-radius:8px; border:1px solid #c7c5d0; background:#f4f8fa; font-weight:600; outline:none;">
        ...
        <div style="font-size:12.5px; color:#0F4C5C; line-height:1.5; background:#d0e6ec; border:1px solid #99c2ce; padding:12px 16px; border-radius:8px; font-weight:600; display:flex; align-items:center; gap:8px;">
  ...
  function updateRuleBadge(id, isValid) {
    ...
    if (isValid) {
      el.style.color = '#0F4C5C';
      if (icon) {
        icon.style.background = '#0F4C5C';
